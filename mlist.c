...
MList *ml_create(void) {
	...
	/* malloc for nodes */
	ml->nodes = malloc(sizeof(Bucket)*ml->size);

	/* malloc for each node */
	for (i = 0; i < size; i++) {
		ml->nodes[i] = malloc(sizeof(Bucket));
...
int ml_add(MList **ml, MEntry *me) {
	...
	if ((bnew = malloc(sizeof(Bucket))) == NULL) {
		return 0;
	}
	bnew->next = NULL;
	...
	b = (*ml)->nodes[hashvalue];

	/* go to its end and find its size */
	bsize = 0;
	while(b->next != NULL) {
		b = b->next;
		bsize++;
